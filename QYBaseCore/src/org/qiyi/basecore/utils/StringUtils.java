...
public class StringUtils {
    ...
    public static final String maskHttpUrl(String strUrl) {
    ...
    public static final Uri maskHttpUri(String strUrl) {
        strUrl = maskHttpUrl(strUrl);
        if (isNotEmpty(strUrl)) {
...
//		return isEmptyArray(array, 1);
    ...
    public static Double string2DoubleScale(String str, int scale) {
        return isEmpty(str) ? 0.0 : new BigDecimal(str).setScale(2,
    ...
    public static Integer toDoubleScale(String str, int scale) {
        ...
        Double _double = new BigDecimal(str).setScale(scale, BigDecimal.ROUND_HALF_UP).doubleValue()
                * 100;
...
//		long ld2= Long.parseLong("1395514448000");
        try {
            if (StringUtils.isEmpty(String.valueOf(now)) || StringUtils.isEmpty(String.valueOf(d))
                    || 0 == now || 0 == d) {
                return "";
            }
...
/*		以前的判断方法，已废弃
 * 		if(!StringUtils.isEmpty(id) && ( id.length() > 9  || (id.length() == 9 && id.substring(0)
 * 		.compareTo("2") > 0) && String.valueOf(id).endsWith("09"))){
			return true;
		}*/

        try {
            if ((!StringUtils.isEmpty(id) && (id.endsWith("09") || id.endsWith("16") || id.equals(
                    "9"))) || !(isEmpty(upderid) || upderid.startsWith("-") || upderid.equals(
                    "0"))) {
                return true;
            }
        } catch (Exception e) {
    ...
    public static String appendGateway(String url, int from_type, int from_sub_type) {
        ...
        DebugLog.v("StringUtils", "appendGateway: " + url);
     ...
     */
    public static String replaceUserInfo(String version, String discover_url, String token,
            String uid) {
        ...
        if (!isEmpty(discover_url)) {
            if (discover_url.contains("?")) {
                ...
                if (discover_url.contains("mobact2rd/acts/1/6/0/")) {
                    discover_url = discover_url.replace(version + "/0/",
                            version + "/" + token + "/");
                }
                if (discover_url.contains("mobact2rd/acts/1/6/0/")) {
     ...
     * 为url添加参数（有则替换）
     */
    public static String appendOrReplaceUrlParameter(String url,
            ...
            for (Map.Entry<String, String> entry : parameterMap.entrySet()) {
                ...
                if (parameterName != null) {
                    ...
                    if (ret.contains(parameterOccurrence = "?" + parameterName + "=")) {
                        ret = ret.replaceAll("\\" + parameterOccurrence + "([^&]+|)",
                                parameterOccurrence + entry.getValue());
        ...
        if (parameterMap != null) {
            ...
            for (Map.Entry<String, String> entry : parameterMap.entrySet()) {
                ...
                if (!isEmpty(parameterName)) {
                    try {
                        appendSB.append(
                                parameterName + "=" + String.valueOf(entry.getValue()) + "&");
                    } catch (Exception e) {
                        DebugLog.e("Stringutils", parameterName);
                        ExceptionUtils.printStackTrace(e);
                    }
                }
            }
     ...
     * 统计基线APP置于后台活动的时间
     */
    public static String getBackgroundInterval(String oldInterval, long interval, int sizeLimit,
            String tag) {
        if (TextUtils.isEmpty(oldInterval)) {
            return String.valueOf(interval);
        }
        String[] intervals = oldInterval.split(tag);
        int length = intervals.length;
        if (length < sizeLimit) {
            return oldInterval + tag + interval;
        } else {
            String newInterval = String.valueOf(interval);
            for (int num = (length - 1); num >= (length + 1 - sizeLimit); num--) {
                newInterval = intervals[num] + tag + newInterval;
            }
            return newInterval;
        }
    }
