...
import org.qiyi.basecore.imageloader.gif.GifFrame;
...
/*                    if (header.currentFrame == null) {
    ...
    private void readBitmap() {
        ...
        int lctSize = (int) Math.pow(2, (packed & 0x07) + 1.0);
        // 3 - sort flag
    ...
    private int[] readColorTable(int ncolors) {
        ...
        try {
            ...
            if (Log.isLoggable(GifDrawable.DEBUGKEY+TAG, Log.DEBUG)) {
                Log.d(GifDrawable.DEBUGKEY+TAG, "Format Error Reading Color Table", e);
            }
            header.status = GifDecoder.STATUS_FORMAT_ERROR;
        }
    ...
    private int readBlock() {
        ...
        if (blockSize > 0) {
            ...
            try {
                ...
                if (Log.isLoggable(GifDrawable.DEBUGKEY+TAG, Log.DEBUG)) {
                    Log.d(GifDrawable.DEBUGKEY+TAG, "Error Reading Block n: " + n + " count: " + count + " blockSize: " + blockSize, e);
                }
                header.status = GifDecoder.STATUS_FORMAT_ERROR;
            }
        }
